    ...
    $('#cancelbtn').on('click', function () {
        $.msgBox({
            ...
            success: function (result) {
                if (result == "OK")
                    location.href = "/EndUserReservation/NewReservation";
            }
        })        
    });
    ...
    $('#confirmbtn').on('click', function () {        
        ...
        if (idType == 0 || idNumber == "" || customerName == "") {
            ...
            if (oldNum == "" && reason == "CHANGE_NUMBER") {
                ...
                $('body').addClass('wait');
                $.ajax({
                    url: "/EndUserReservation/ReserveNumber",
                    data: { number: num, customerName: customerName, typeID: idType, idNumber: idNumber, contractNO: contactNum, prof: prof, educ: educ, birthDate: birthDate, alternativeNumber: alterNum, reason: reason, oldNumber: oldNum, remark: remark },
                    type: "GET",
                    dataType: "json",
                    cache: false,
                    async: true,
