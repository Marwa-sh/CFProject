$(window).load(function () {
    ...
    $().loadMore({
        ...
        auto: false,
        url: "/FreezedSeries/groupFreezedSeriesF?" + url,
    });
    ...
    $(document).on('click', '#unfreezeSelectedGroup', function (e) {
        ...
        if (count == 0) {
            ...
            $.msgBox({
                title: 'Unfreeze Groups Of Series',
                content: 'Are You Sure you want to unfreeze ' + count + 'group(s) of series? ',
                type: "confirm",
                opacity: 0.5,
                                    ...
                                    $.ajax({
                                        ...
                                        success: function (data) {
                                            if (data == false)
                                                ...
                                                $('body').addClass('wait');
                                                $.ajax({
                                                    url: "/FreezedSeries/unfreezeSelectedGroupSeries?"+url,
                                                    type: "GET",
                                                    ...
                                                    success:
                                                        function (data) {
                                                            if (data.status) {
                                                                ...
                                                                $.msgBox({
                                                                    title: 'WARNING',
                                                                    content: "Unfreeze groups of series failed.",
                                                                    opacity: 0.5,
                                                                    buttons: [{ value: "OK" }],
                                                                })
                                                        },
    ...
    $(document).on('click', '#unfreezeAllGroup', function (e) {
        ...
        else
        {
            $.msgBox({
                title: 'Unfreeze Groups Of Series',
                content: 'Are You Sure you want to unfreeze all groups of series? ',
                type: "confirm",
                opacity: 0.5,
                buttons: [{ value: "Yes" }, { value: "No" }],
                success: function (result) {
